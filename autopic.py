...
from pyOreo.fns.helper import download_file
from pyOreo.fns.tools import get_google_images

from . import LOGS, get_help, get_string, udB, oreo_bot, oreo_cmd

__doc__ = get_help("help_autopic")


@oreo_cmd(pattern="autopic( (.*)|$)")
async def autopic(e):
...
if search := udB.get_key("AUTOPIC"):
    ...
    async def autopic_func():
        ...
        file = await oreo_bot.upload_file(filee)
        await oreo_bot(UploadProfilePhotoRequest(file))
        os.remove(filee)
