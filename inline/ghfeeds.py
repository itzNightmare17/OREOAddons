...
async def gh_feeds(ore):
    try:
        username = ore.text.split(maxsplit=1)[1]
    except IndexError:
        return await ore.answer(
            [],
    ...
    for cont in data[:50]:
        ...
        article = await ore.builder.article(
            title=title,
            text=text,
            url=repo_url,
            parse_mode="html",
            ...
            buttons=[
                Button.url("View", url),
                Button.switch_inline("Search again", query=ore.text, same_peer=True),
            ],
        )
    ...
    await ore.answer(res, cache_time=5000, switch_pm=msg, switch_pm_param="start")
