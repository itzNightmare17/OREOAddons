...
async def koo_search(ore):
    """Search Users on koo with API"""
    try:
        match = ore.text.split(maxsplit=1)[1].lower()
        match_ = match
    except IndexError:
                ...
                await ore.builder.article(
                    ...
                    buttons=[
                        ...
                        Button.switch_inline(
                            "• Share •",
                            query=ore.text if key_count else f"{ore.text} | {count}",
                        ),
                    ],
                )
            )
    ...
    await ore.answer(res, switch_pm=switch, switch_pm_param="start")
