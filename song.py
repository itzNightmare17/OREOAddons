...
@oreo_cmd(pattern="song ?(.*)")
async def _(event):
    oreo_bot = event.client
    try:
        await oreo_bot(ImportChatInviteRequest("DdR2SUvJPBouSW4QlbJU4g"))
    ...
    try:
        async for event in oreo_bot.iter_messages(
            chat, search=args, limit=1, filter=filtermus
        ):
            await oreo_bot.send_file(current_chat, event, caption=event.message)
        await okla.delete()
    except Exception:
